...
template<typename T>
void AvlTree<T>::insertNode(T* node) {
  _insert(root, node);
}

template<typename T>
T* AvlTree<T>::_insert(T* root, T* node) {
 if (root == nullptr) {
    return node; 
  }

  if (node < root) {
    root->left_child = _insert(root->left_child, node);
  } else if (node > root) {
    root->right_child = _insert(root->right_child, node);
  } else {
...
template<typename T>
void AvlTree<T>::deleteNode(T* node) {
  _delete(root, node);
}
